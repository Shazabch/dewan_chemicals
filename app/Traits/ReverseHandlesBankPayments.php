...
trait ReverseHandlesBankPayments
{
        ...
        if ($method === 'cash' || $method === 'both') {
            ...
            if ($cashBank) {
                $this->handleBankTransaction(
                    ...
                    $dataModelId,
                    $dataModelName,
                    $method === 'both' ? 'Reverse Cash Payment for '.$dataModelName.' (Both)' : 'Reverse Cash Payment '.$dataModelName,
                    $transaction_type,
                );
            }
        }
        ...
        if ($method === 'bank' || $method === 'both') {
            ...
            if ($selectedBank) {
                $this->handleBankTransaction(
                    ...
                    $dataModelId,
                    $dataModelName,
                    $method === 'both' ? 'Reverse Bank Payment for '.$dataModelName.' (Both)' : 'Reverse Bank Payment '.$dataModelName,
                    $transaction_type,
                );
            }
        }
